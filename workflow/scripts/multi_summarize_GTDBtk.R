...
stats_file         <- snakemake@input[["bins"]] # "Dereplication/dRep_multi/allSamples_bin_stats.tsv"
out_files            <- snakemake@output #"dereplicated_rebin_stats.tsv","all_rebin_stats.tsv"
gtdb_dirs         <- snakemake@input[["gtdb"]] #"Dereplication/GTDB_multi/{i}.GTDB_out"

###################################################################################################
## Read dRep output for all bins
###################################################################################################
print("reading dRep output")
print("Chdb.csv")
...
wins$memberNames <- sapply(wins$secondary_cluster,
                           function(x) {
                             ...
                             r})
print("Ndb.csv")
dist <- read.delim(cluster_files[[6]],sep=",",stringsAsFactors = F)
...
###################################################################################################
print("Read stats")
stat <- read.delim(stats_file[[1]],stringsAsFactors = F)
...
bins <- merge(bins,allS,by=1,all.x=T,suffixes=c(".dRep",".all"))
wins <- merge(wins,allS,by=1,all.x=T,suffixes=c(".dRep",".all"))

###################################################################################################
## Read GTDBtk output for selected bins
###################################################################################################
print("Reading GTDBtk results.")
for(bin in wins$genome){
  currdir <- paste0("Dereplication/GTDB_multi/",bin,".GTDB_out")
  if(currdir %in% unlist(gtdb_dirs)){
    if(file.exists(currdir)){
      tfile <- paste0(currdir,"/",
                      list.files(path=currdir,
                               pattern="[^_]summary.tsv"))
      if(!"gtdbTab" %in% ls()){
        gtdbTab <- data.frame("bin"=bin,
                              read.delim(tfile,
                                       stringsAsFactors=F)[,-1],
                            stringsAsFactors = F)
      }else{
        gtdbTab <- rbind(gtdbTab,
                       data.frame("bin"=bin,
                                  read.delim(tfile,
                                             stringsAsFactors=F)[,-1],
                                  stringsAsFactors = F))
...
wins <- merge(wins,gtdbTab,by=1,all.x=T,suffixes=c("",".GTDBtk"))
